    ...
    useEffect(() => {
        ...
        async function loadActiveQuestions() {
            ...
            try {
                ...
                if (mode === 'tournament' && tournamentId) {
                    const { getTournamentQuestionSetSelection } = await import('@/app/actions/tournament');
                    const selection = await getTournamentQuestionSetSelection(tournamentId, 'pixel-runner');

                    if (!selection.success || !selection.questionSetId) {
                        console.warn("Tournament question set unavailable for pixel-runner:", selection.error);
                    } else {
                        const { getQuestions } = await import('@/app/actions/game-data');
                        questions = await getQuestions(selection.questionSetId);
                    }
                }

                if (!questions) {
                    const { getRuntimeQuestions } = await import('@/app/actions/game-data');
                    const runtime = await getRuntimeQuestions('pixel-runner');
                    questions = runtime.success ? runtime.questions : [];
                }

                if (questions && questions.length > 0) {
                    const validQuestions = (questions as QuizQuestionSource[])
                        .map((q) => {
                    ...
                    if (validQuestions.length > 0) {
                        QUIZ_DATA.splice(0, QUIZ_DATA.length, ...validQuestions.map((q) => ({
                            q: typeof q.question_text === "string" ? q.question_text : "",
                            o: (q.options as string[]),
                            a: typeof q.correct_answer === "number" ? q.correct_answer : 0
                        })));
        ...
        class Player {
            ...
            die() {
                ...
                setTimeout(() => {
                    handleGameEnd("GAME OVER", "#ff4444");
                }, 2500);
            }
        ...
        <div className="game-body-wrapper font-pixel text-white flex flex-col justify-center items-center min-h-screen bg-[#111] overflow-hidden">
            ...
            <style jsx global>{`
                ...
                #startScreen {
                    ...
                    background: radial-gradient(circle at center, rgba(30,15,45,0.9) 0%, rgba(5,0,15,1) 100%);
                    display: flex;
                ...
                .retro-title {
                    ...
                    text-shadow:
                        ...
                        4px 4px 0 #00eaff;
                    margin-bottom: 15px;
                    animation: titleFloat 3s ease-in-out infinite, pulseNeon 2s infinite alternate;
                    line-height: 1.2;
                    letter-spacing: 2px;
                    z-index: 2;
                }
                ...
                .retro-desc {
                    ...
                    border-radius: 8px;
                    box-shadow: 0 0 15px rgba(255,0,222,0.1);
                    z-index: 2;
                }
                ...
                .press-start:hover {
                    ...
                    box-shadow: 0 0 20px #ff00de, inset 0 0 20px #ff00de, 0 0 40px #00eaff;
                    border-color: #00eaff;
                    text-shadow: 0 0 5px #fff, 0 0 10px #00eaff;
                    transform: scale(1.05);
                }

                .press-start:hover::after {
                    transform: rotate(45deg) translateY(100%);
                }

                @keyframes titleFloat { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-15px); } }
                @keyframes pulseNeon { 0% { opacity: 1; } 100% { opacity: 0.85; } }
                @keyframes gridMove { 0% { background-position: 0 0; } 100% { background-position: 0 40px; } }
                @keyframes buttonPulse {
                    0%, 100% { box-shadow: 0 0 10px #ff00de, inset 0 0 10px #ff00de; }
                    50% { box-shadow: 0 0 20px #ff00de, inset 0 0 15px #ff00de; }
                }
            ...
            <div id="gameContainer" ref={containerRef}>
                ...
                <div id="startScreen">
                    ...
                    <div className="retro-subtitle">CYBER CITY DASH</div>
                    <div className="retro-desc text-center">
                        <span className="text-[#00eaff] mb-2 block">◆ CONTROLS ◆</span>
                        PC: [SPACE] JUMP / [DOWN] SLIDE<br />
                        MOBILE: USE BUTTONS BELOW
                    </div>
                    <button id="startBtn" className="press-start" onClick={() => startRef.current?.()}>START GAME</button>

                    {/* Decorative elements */}
                    <div className="absolute top-4 left-4 flex gap-2 w-full max-w-[150px] opacity-50 z-10">
                        <div className="h-1 w-full bg-[#ff00de]" />
                        <div className="h-1 w-4 bg-[#00eaff]" />
                        <div className="h-1 w-2 bg-[#ff00de]" />
                    </div>
                    <div className="absolute bottom-4 right-4 flex gap-2 w-full max-w-[150px] opacity-50 z-10 flex-row-reverse">
                        <div className="h-1 w-full bg-[#ff00de]" />
                        <div className="h-1 w-4 bg-[#00eaff]" />
                        <div className="h-1 w-2 bg-[#ff00de]" />
                    </div>
